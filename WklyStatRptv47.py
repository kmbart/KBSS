...
# for each team in the league create empty season stats list
    for team in teamAbbrList:
        seasonStats[team] = copy.deepcopy(teamStatsTemplate)

#     print ('season stats at start:', seasonStats)
...
# initialize the stats dictionary for this team
            teamStatsDict[teamAbbr] = copy.deepcopy(teamStatsTemplate)
...
#                print ('currStatus:', currStatus, 'oldStatus:', oldStatus)
                if oldStatus != currStatus:
                    if oldStatus == 1:
                        tempYTDPitcherList.append(makeTeamPitcherTotalsStr(teamStatsDict, teamAbbr))
                        tempYTDHitterList.append(makeTeamHitterTotalsStr(teamStatsDict, teamAbbr))
...
# find difference of current and previous week's stats
                    ...
                    if playerType == 'P':
                        if currStatus == 1:
                            sumPitcherStats(teamStatsDict, teamAbbr, currWKStats)
                            sumPitcherStats(seasonStats, teamAbbr, currWKStats)
...
#                            print ('pID=', playerID, ',prevPos=', prevPos)
                        ...
                        if currStatus == 1:
                            sumHitterStats(teamStatsDict, teamAbbr, currWKStats)
                            sumHitterStats(seasonStats, teamAbbr, currWKStats)
...
# create weekly category lists and then calculate points based on those lists
        PWList = sorted([(t[0], t[1]['P_Wins']) for t in teamStatsDict.items()], key=lambda k: k[1], reverse=True)
        putPointsOnList(numOfTeams, PWList)
        PSList = sorted([(t[0], t[1]['P_Saves']) for t in teamStatsDict.items()], key=lambda k: k[1], reverse=True)
        putPointsOnList(numOfTeams, PSList)
        PKList = sorted([(t[0], t[1]['P_K']) for t in teamStatsDict.items()], key=lambda k: k[1], reverse=True)
        putPointsOnList(numOfTeams, PKList)
        PERAList = sorted([(t[0], float((t[1]['P_ERuns'] * 9) / t[1]['P_IP'])) for t in teamStatsDict.items()],
                          key=lambda k: k[1])
        putPointsOnList(numOfTeams, PERAList)
        PWHIPList = sorted([(t[0], float((t[1]['P_Hits'] + t[1]['P_BB']) / t[1]['P_IP'])) for t in teamStatsDict.items()],
                           key=lambda k: k[1])
        putPointsOnList(numOfTeams, PWHIPList)
#        print ('PWHIPL:', PWHIPList)

        HHRList = sorted([(t[0], t[1]['H_HR']) for t in teamStatsDict.items()], key=lambda k: k[1], reverse=True)
        putPointsOnList(numOfTeams, HHRList)
        HRBIList = sorted([(t[0], t[1]['H_RBI']) for t in teamStatsDict.items()], key=lambda k: k[1], reverse=True)
        putPointsOnList(numOfTeams, HRBIList)
        HRUNSList = sorted([(t[0], t[1]['H_Runs']) for t in teamStatsDict.items()], key=lambda k: k[1], reverse=True)
        putPointsOnList(numOfTeams, HRUNSList)
        HSBList = sorted([(t[0], t[1]['H_SB']) for t in teamStatsDict.items()], key=lambda k: k[1], reverse=True)
        putPointsOnList(numOfTeams, HSBList)
        HBAList = sorted([(t[0], float(t[1]['H_Hits'] / t[1]['AB'])) for t in teamStatsDict.items()], key=lambda k: k[1],
                         reverse=True)
...
PWList = sorted([(t[0], t[1]['P_Wins']) for t in seasonStats.items()], key=lambda k: k[1], reverse=True)
putPointsOnList(numOfTeams, PWList)
PSList = sorted([(t[0], t[1]['P_Saves']) for t in seasonStats.items()], key=lambda k: k[1], reverse=True)
putPointsOnList(numOfTeams, PSList)
PKList = sorted([(t[0], t[1]['P_K']) for t in seasonStats.items()], key=lambda k: k[1], reverse=True)
putPointsOnList(numOfTeams, PKList)
PERAList = sorted([(t[0], float((t[1]['P_ERuns'] * 9) / t[1]['P_IP'])) for t in seasonStats.items()],
                  key=lambda k: k[1])
putPointsOnList(numOfTeams, PERAList)
PWHIPList = sorted([(t[0], float((t[1]['P_Hits'] + t[1]['P_BB']) / t[1]['P_IP'])) for t in seasonStats.items()],
                   key=lambda k: k[1])
putPointsOnList(numOfTeams, PWHIPList)

HHRList = sorted([(t[0], t[1]['H_HR']) for t in seasonStats.items()], key=lambda k: k[1], reverse=True)
putPointsOnList(numOfTeams, HHRList)
HRBIList = sorted([(t[0], t[1]['H_RBI']) for t in seasonStats.items()], key=lambda k: k[1], reverse=True)
putPointsOnList(numOfTeams, HRBIList)
HRUNSList = sorted([(t[0], t[1]['H_Runs']) for t in seasonStats.items()], key=lambda k: k[1], reverse=True)
putPointsOnList(numOfTeams, HRUNSList)
HSBList = sorted([(t[0], t[1]['H_SB']) for t in seasonStats.items()], key=lambda k: k[1], reverse=True)
putPointsOnList(numOfTeams, HSBList)
HBAList = sorted([(t[0], float(t[1]['H_Hits'] / t[1]['AB'])) for t in seasonStats.items()], key=lambda k: k[1],
                 reverse=True)
putPointsOnList(numOfTeams, HBAList)
...
for ix in range(0, numOfTeams):
    print('  ', pitRankList[ix][0], "{: 10.2f}".format(pitRankList[ix][1][0]), '   ',
          ...
          totRankList[ix][0], "{: 10.2f}".format(totRankList[ix][1][2]), file=seasonStatsFile)

# print ('season stats:', seasonStats)
