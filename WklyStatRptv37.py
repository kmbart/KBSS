...
#    print ('leagstats:', leagueWklyStats)
...
# Generate dictionary of previous season's positions for all players
    _SQL = 'select ID, FirstName, LastName, GamesAtC, GamesAt1B, GamesAt2B, GamesAt3B, GamesAtSS, GamesAtOF from WklyStats where CCYYMMDD = ?'
    lastWk = int(prevSeasonLastWeek)
    print ('prevLastWk:', lastWk)
...
#                                teamPhone, ',', teamEmail, ',', teamSkipper)
            ...
            leagueWklyStats[teamAbbr] = copy.deepcopy(statsTemplate)
            ...
            for player in roster:
                ...
                if oldStatus != currStatus:
                    if oldStatus == 1:
                        tempPList.append(makeTeamPitcherTotalsStr(leagueWklyStats, teamAbbr))
                        tempHList.append(makeTeamHitterTotalsStr(leagueWklyStats, teamAbbr))
...
#                print ('pInfo:', playerInfoStr)
                ...
                if currStats == None:
                    ...
                    if playerType == 'P':
                        sumPitcherStats(leagueWklyStats, teamAbbr, statDiff)
                        ...
                        sumHitterStats(leagueWklyStats, teamAbbr, statDiff)
...
# print ('League Stats:', leagueWklyStats)

PWList = sorted([(t[0], t[1]['P_Wins']) for t in leagueWklyStats.items()], key=lambda k: k[1], reverse=True)
putPointsOnList(numOfTeams, PWList)
PSList = sorted([(t[0], t[1]['P_Saves']) for t in leagueWklyStats.items()], key=lambda k: k[1], reverse=True)
putPointsOnList(numOfTeams, PSList)
PKList = sorted([(t[0], t[1]['P_K']) for t in leagueWklyStats.items()], key=lambda k: k[1], reverse=True)
putPointsOnList(numOfTeams, PKList)
PERAList = sorted([(t[0], float((t[1]['P_ERuns'] * 9) / t[1]['P_IP'])) for t in leagueWklyStats.items()],
                  key=lambda k: k[1])
putPointsOnList(numOfTeams, PERAList)
PWHIPList = sorted([(t[0], float((t[1]['P_Hits'] + t[1]['P_BB']) / t[1]['P_IP'])) for t in leagueWklyStats.items()],
                   key=lambda k: k[1])
putPointsOnList(numOfTeams, PWHIPList)

HHRList = sorted([(t[0], t[1]['H_HR']) for t in leagueWklyStats.items()], key=lambda k: k[1], reverse=True)
putPointsOnList(numOfTeams, HHRList)
HRBIList = sorted([(t[0], t[1]['H_RBI']) for t in leagueWklyStats.items()], key=lambda k: k[1], reverse=True)
putPointsOnList(numOfTeams, HRBIList)
HRUNSList = sorted([(t[0], t[1]['H_Runs']) for t in leagueWklyStats.items()], key=lambda k: k[1], reverse=True)
putPointsOnList(numOfTeams, HRUNSList)
HSBList = sorted([(t[0], t[1]['H_SB']) for t in leagueWklyStats.items()], key=lambda k: k[1], reverse=True)
putPointsOnList(numOfTeams, HSBList)
HBAList = sorted([(t[0], float(t[1]['H_Hits'] / t[1]['AB'])) for t in leagueWklyStats.items()], key=lambda k: k[1],
                 reverse=True)
...
print('   Wins        ', 'Saves      ', 'Strikeouts        ', 'ERA             ', 'WHIP')
for ix in range(0, numOfTeams):
    print(PWList[ix][0], "{: 4d}".format(PWList[ix][1]), '   ',
          ...
          PWHIPList[ix][0], "{: 7.4f}".format(PWHIPList[ix][1]))

print()
print('    HR           ', 'RBI         ', 'Runs          ', 'SB            ', 'BA')
for ix in range(0, numOfTeams):
    print(HHRList[ix][0], "{: 4d}".format(HHRList[ix][1]), '   ',
          ...
          HBAList[ix][0], "{: 7.4f}".format(HBAList[ix][1]))

print()
print('Pitching Points    ', 'Hitting Points      ', 'Total Points')
for ix in range(0, numOfTeams):
