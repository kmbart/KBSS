...
PERAList = sorted([(t[0], float((t[1]['P_ERuns'] * 9) / t[1]['P_IP'])) for t in leagueStats.items()],
                  key=lambda k: k[1])
putPointsOnList(numOfTeams, PERAList)
PWHIPList = sorted([(t[0], float((t[1]['P_Hits'] + t[1]['P_BB']) / t[1]['P_IP'])) for t in leagueStats.items()],
                   key=lambda k: k[1])
...
HBAList = sorted([(t[0], float(t[1]['H_Hits'] / t[1]['AB'])) for t in leagueStats.items()], key=lambda k: k[1],
                 reverse=True)
