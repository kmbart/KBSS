...
def getFileType (s, p):                      # status, player type
    if s > 1:
        fType = 'E'
    elif p == 'P':
        fType = 'P'
...
def printHeadersHitters(f):                                    # file type
...
def printHeadersPitchers(f):                                   # file type
    print ("{:^30}".format('Player')
           ,"{:^7}".format('ID')
           ,"{:^5}".format('$')
           ,"{:^4}".format('Con')
...
# print hitter stats
def printHitterStats(s, e, f):                                 # stats, positions, file type
    print (' '
           ,"{: 4d}".format(s[6])                              # AB
           ,"{: 4d}".format(s[8])                              # hits
           ,"{: 4d}".format(s[11])                             # HR
...
# print pitcher stats
def printPitcherStats(s, f):                                   # stats, file type
    print (' '
           ,"{: 3d}".format(s[24])                             # wins
           ,"{: 3d}".format(s[26])                             # saves
           ,"{: 6.1f}".format(s[28])                           # innings pitched
           ,"{: 3d}".format(s[29])                             # hits
           ,"{: 3d}".format(s[31])                             # walks
           ,"{: 3d}".format(s[34])                             # earned runs
           ,"{: 3d}".format(s[35])                             # strikeouts
           ,"{: 7.3f}".format(9 * (s[34] / s[28]))             # ERA
           ,"{: 7.3f}".format((s[29] + s[31]) / s[28])         # WHIP
        ,file = f)

# print player info
def printPlayerInfo(p, f):                                     # player, file
    print (p[0][0],                                            # status
           p[1],                                               # position
...
def printTeamHitterTotals(l, t, f):                            # league, stat totals, file type
    print (t, ' - Hitter Totals:                         '     # team abbr
           ,"{: 5d}".format(l[t]['AB'])                        # AB
           ,"{: 4d}".format(l[t]['H_Hits'])                    # hits
...
# print printer stat totals for a team
def printTeamPitcherTotals(l, t, f):                           # league, stats totals, file type
    print (t, ' - Pitcher Totals:                         '                     # team abbr
           ,"{: 3d}".format(l[t]['P_Wins'])                                     # wins
           ,"{: 3d}".format(l[t]['P_Saves'])                                    # saves
           ,"{: 6.1f}".format(l[t]['P_IP'])                                     # IP
           ,"{: 3d}".format(l[t]['P_Hits'])                                     # hits
           ,"{: 3d}".format(l[t]['P_BB'])                                       # walks
...
# open temp team file, temp pitcher file, temp hitter file
            tempEFile = open('tempextrafile.txt', 'w')
            tempHFile = open('temphitfile.txt', 'w')
            tempPFile = open('temppitfile.txt', 'w')
            tempTFile = open('tempteamfile.txt', 'w')
...
#                               teamPhone, ',', teamEmail, ',', teamSkipper)
            ...
            print (teamName, ' (', teamAbbr, ') ', teamOwner, ',',
                               teamPhone, ',', teamEmail, ',', teamSkipper
                ,file = tempTFile)
...
# check for break of active pitchers or hitters - change in status or change in position
                if oldStatus == 1:
                    if oldStatus != currStatus:
                        tempFile = tempHFile
                        printTeamHitterTotals(leagueStats, teamAbbr, tempFile)
                   
                    elif oldPosition != currPosition and oldPosition == '01':
                        tempFile = tempPFile
                        printTeamPitcherTotals(leagueStats, teamAbbr, tempFile)
                        tempFile = tempHFile
                        printHeadersHitters(tempFile)

                oldStatus   = currStatus
                oldPosition = currPosition

                fileType = getFileType(currStatus, playerType)
#                print ('f type:', fileType)
                ...
                printPlayerInfo(player, tempFile)
                        
                if currStats == None:
                    print ('  <No stats returned>', file = tempFile)
...
# find difference of current and previous week's stats
                    ...
                    if playerType == 'P':
                        ...
                        printPitcherStats(currStats, tempFile)
                    else:
                        ...
                        printHitterStats(currStats, currPos, tempFile)
        
            print (' ', file = tempEFile)
            print ('End of Team:', team, ' for week:', week)
            print ('End of Team:', team, file = tempEFile)
            print (' ', file = tempEFile)
...
# re-open temp files, copy contents to stat file and close again
            tempEFile = open('tempextrafile.txt', 'r')
            tempHFile = open('temphitfile.txt', 'r')
            tempPFile = open('temppitfile.txt', 'r')
            tempTFile = open('tempteamfile.txt', 'r')
        
            for aLine in tempTFile: print (aLine, file=statsFile, end='')
            for aLine in tempPFile: print (aLine, file=statsFile, end='')
            for aLine in tempHFile: print (aLine, file=statsFile, end='')
            print (' ', file = statsFile)
            print (team, ' - Reserved and Minor League Players', file = statsFile)
            for aLine in tempEFile: print (aLine, file=statsFile, end='')
