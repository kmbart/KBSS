...
def printPlayerInfo(p):
    if playerType == 'P': tempFile = tempPFile
    else: tempFile = tempHFile
...
def printTeamPitcherTotals(l, t):
...
# open temp team file, temp pitcher file, temp hitter file
            ...
            tempMFile = open('tempminfile.txt', 'w')
            tempPFile = open('temppitfile.txt', 'w')
            tempRFile = open('tempresfile.txt', 'w')
...
# check for break of active pitchers or hitters - change in status or change in position
                if oldStatusOrder == 10:
                    if oldStatusOrder != statusOrder[player[0]]:
                        printTeamHitterTotals(leagueStats, teamAbbr)
                   
                    elif oldPositionOrder != positionOrder[player[1]] and oldPositionOrder == '01':
                        printTeamPitcherTotals(leagueStats, teamAbbr)
                        printHeadersHitters()

                oldStatusOrder   = statusOrder[player[0]]
                oldPositionOrder = positionOrder[player[1]]
...
#                print (); print ('currstats:', currStats); print ()
                ...
                if currStats == None:
                    if oldPositionOrder == '01':
                        playerType = 'P'
                    else:
                        playerType = 'H'
#                    print ('no stats player type:', playerType)
                        
                    printPlayerInfo(player)
                    if playerType == 'P':
                        print ('  <No stats returned>', file = tempPFile)
                    else:
                        print ('  <No stats returned>', file = tempHFile)
...
# open temp files and copy to stat file
        tempHFile = open('temphitfile.txt', 'r')
        tempMFile = open('tempminfile.txt', 'r')
        tempPFile = open('temppitfile.txt', 'r')
        tempRFile = open('tempresfile.txt', 'r')
        ...
        for aLine in tempRFile: print (aLine, file=statsFile, end='')
        for aLine in tempMFile: print (aLine, file=statsFile, end='')
