...
# PKList = {teamKey : {(catgVal) for (catgKey, catgVal) in teamVal.items() if catgKey == 'P_K'} for (teamKey, teamVal) in leagueStats.items()}
# print ('PK:', PKList)
# PWList = {teamKey : {(catgVal) for (catgKey, catgVal) in teamVal.items() if catgKey == 'P_Wins'} for (teamKey, teamVal) in leagueStats.items()}
# print ('PW:', PWList)

# HHRList = {teamKey : {(catgVal) for (catgKey, catgVal) in teamVal.items() if catgKey == 'H_HR'} for (teamKey, teamVal) in leagueStats.items()}
# print ('HHR:', HHRList)
# HSBList = {teamKey : {(catgVal) for (catgKey, catgVal) in teamVal.items() if catgKey == 'H_SB'} for (teamKey, teamVal) in leagueStats.items()}
# HSBList = [teamKey, catgVal] for (catgKey, catgVal) in teamVal.items() if catgKey == 'H_SB'
# print ('HSB:', HSBList)
# tList = [(t[0], t[1]['H_SB']) for t in leagueStats.items()]
# tList.sort(key = lambda k: k[1], reverse=True)
tList = sorted ([(t[0], t[1]['P_Wins']) for t in leagueStats.items()], key = lambda k: k[1], reverse=True)
print ('tList:', tList)
tList = sorted ([(t[0], t[1]['P_Saves']) for t in leagueStats.items()], key = lambda k: k[1], reverse=True)
print ('tList:', tList)
tList = sorted ([(t[0], t[1]['P_K']) for t in leagueStats.items()], key = lambda k: k[1], reverse=True)
print ('tList:', tList)
# ERA tList = sorted ([(t[0], t[1]['P_SB']) for t in leagueStats.items()], key = lambda k: k[1], reverse=True)
# WHIP tList = sorted ([(t[0], t[1]['P_SB']) for t in leagueStats.items()], key = lambda k: k[1], reverse=True)

tList = sorted ([(t[0], t[1]['H_HR']) for t in leagueStats.items()], key = lambda k: k[1], reverse=True)
print ('tList:', tList)
tList = sorted ([(t[0], t[1]['H_RBI']) for t in leagueStats.items()], key = lambda k: k[1], reverse=True)
print ('tList:', tList)
tList = sorted ([(t[0], t[1]['H_Runs']) for t in leagueStats.items()], key = lambda k: k[1], reverse=True)
print ('tList:', tList)
tList = sorted ([(t[0], t[1]['H_SB']) for t in leagueStats.items()], key = lambda k: k[1], reverse=True)
# BA tList = sorted ([(t[0], t[1]['H_SB']) for t in leagueStats.items()], key = lambda k: k[1], reverse=True)
print ('tList:', tList)
