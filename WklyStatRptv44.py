...
# for each team in the league create empty season stats list
    for team in teamAbbrList:
        seasonStats[team] = copy.deepcopy(statsTemplate)
...
# find difference of current and previous week's stats
                    ...
                    if playerType == 'P':
                        if currStatus == 1:
                            ...
                            sumPitcherStats(seasonStats, teamAbbr, statDiff)
...
#                            print ('pID=', playerID, ',prevPos=', prevPos)
                        ...
                        if currStatus == 1:
                            ...
                            sumHitterStats(seasonStats, teamAbbr, statDiff)
...
# print ('sfilename:', statsFilename)
        ...
        for ix in range(0, numOfTeams):
            print('  ', pitRankList[ix][0], "{: 10.1f}".format(pitRankList[ix][1][0]), '   ',
                  ...
                  totRankList[ix][0], "{: 10.1f}".format(totRankList[ix][1][2]), file=weeklyStatsFile)
...
# create season standings
print('   Wins        ', 'Saves      ', 'Strikeouts        ', 'ERA             ', 'WHIP', file=seasonStatsFile)
for ix in range(0, numOfTeams):
    print(PWList[ix][0], "{: 4d}".format(PWList[ix][1]), '   ',
          ...
          PWHIPList[ix][0], "{: 7.4f}".format(PWHIPList[ix][1]), file=seasonStatsFile)

print(' ', file=seasonStatsFile)
print('    HR           ', 'RBI         ', 'Runs          ', 'SB            ', 'BA', file=seasonStatsFile)
for ix in range(0, numOfTeams):
    print(HHRList[ix][0], "{: 4d}".format(HHRList[ix][1]), '   ',
          ...
          HBAList[ix][0], "{: 7.4f}".format(HBAList[ix][1]), file=seasonStatsFile)

# reset season category lists
pointTotals = {}
for ix in PWList:
    pointTotals[ix[0]] = [ix[2], 0, ix[2]]
for ix in PSList:
