NOT USED ANY MORE -
  NOW LOADING ALL HITTERS INTO A SINGLE TABLE BECAUSE PYTHON CANNOT USE TABLENAMES AS PARMS
#  LoadNLH - get NLH for each week in a single season
#
#  set the weeks in each season (MMDD format) as a tuple
#  set current year for season (CCYY format) and weeks tuple to be processed (MMDD format)
#  set database filename to RWDB.db
#  set folder path for season from basic path and current year
#  
#  initialize counts
...
#    set the hitter filename
#    FileName = 'NLH' + SeasonCCYY [3:4] + Week + '.txt'
