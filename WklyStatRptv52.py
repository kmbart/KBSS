...
# attach tiebreaker calcs to tied teams
tieList = []
for ix in range(0, numOfTeams):
    teamIx = totRankList [ix][0]

#    print ('ix:', ix, 'team:', totRankList [ix][0], 'totPts:', totRankList [ix][1][2])
#    print('? ip:', seasonStatsDict[teamIx]['P_IP'], 'ab:', seasonStatsDict[teamIx]['AB'])
#    print ('team:', teamIx, 'team1', teamIx1, 'top:', totRankList[ix][1][2], 'bot:', totRankList[ix + 1][1][2])

    ipNum = int (round (3 * seasonStatsDict[teamIx]['P_IP'],0))
    abNum = seasonStatsDict[teamIx]['AB']
    tieStr = str ('3*IP + AB = '
                    + str (ipNum)
                    + ' + '
                    + str (abNum)
